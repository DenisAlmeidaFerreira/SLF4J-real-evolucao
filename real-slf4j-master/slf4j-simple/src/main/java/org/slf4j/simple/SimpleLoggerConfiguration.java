...
public class SimpleLoggerConfiguration {
    ...
    private String dateTimeFormatStr = DATE_TIME_FORMAT_STR_DEFAULT;
    ...
    private static OutputChoice computeOutputChoice(String logFile, boolean cacheOutputStream) {
        if ("System.err".equalsIgnoreCase(logFile)) {
            return OutputChoice.forSysErr(cacheOutputStream);
        } else if ("System.out".equalsIgnoreCase(logFile)) {
            return OutputChoice.forSysOut(cacheOutputStream);
        } else {
            try {
                ...
                try {
                    ...
                    Reporter.error("UTF-8 encoding not supported. Defaulting to system encoding.", e);
                    printStream = new PrintStream(fos, true);
                }
                return OutputChoice.forFile(printStream);
            } catch (FileNotFoundException e) {
                Reporter.error("Could not open [" + logFile + "]. Defaulting to System.err", e);
                return OutputChoice.forSysErr(false);
            }
        }
    }
